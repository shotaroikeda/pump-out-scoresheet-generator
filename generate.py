...
def add_border(ws, row, column, left=None, right=None, bottom=None, top=None):
    ...
    if top == None:
        top = old.top
        ws.cell(row=row, column=column).border = Border(left=left, right=right, bottom=bottom, top=top,
                                                        diagonal=old.diagonal, diagonal_direction=old.diagonal_direction, vertical=old.vertical, horizontal=old.horizontal)
...
def write_score_sheet(ws, db, chart_set, config, scores):
    ...
    for i, cid in enumerate(charts):
        ...
        ws.cell(row=i + 2, column=2,
                value="=VLOOKUP(A%d, 'Data (Complete)'!A1:O9999, 4, FALSE)" % (i + 2)).fill = gray
        ws.cell(row=i + 2, column=3,
                value="=VLOOKUP(A%d, 'Data (Complete)'!A1:O9999, 5, FALSE)" % (i + 2)).fill = gray
        ws.cell(row=i + 2, column=4,
                value="=VLOOKUP(A%d, 'Data (Complete)'!A1:O9999, 6, FALSE)" % (i + 2)).fill = gray
        ws.cell(row=i + 2, column=5,
                value="=VLOOKUP(A%d, 'Data (Complete)'!A1:O9999, 7, FALSE)" % (i + 2)).fill = gray

        for j, mid in enumerate(mixes):
            ws.cell(row=i + 2, column=col_mix + j,
                    value="NY"[db.chart_in_mix(cid, mid)]).fill = gray
            ws.cell(row=i + 2, column=col_mix + len(mixes),
                    value=db.chart_rating_sequence_str(cid, changes_only=True)).fill = gray
    ...
    if config.pad or config.keyboard:
        ws.conditional_formatting.add('F2:F9999', CellIsRule(
            operator='equal', formula=['"Y"'], fill=green))
        ws.conditional_formatting.add('F2:F9999', CellIsRule(
            operator='equal', formula=['"N"'], fill=red))

    if config.pad and config.keyboard:
        ws.conditional_formatting.add('J2:J9999', CellIsRule(
            operator='equal', formula=['"Y"'], fill=green))
        ws.conditional_formatting.add('J2:J9999', CellIsRule(
            operator='equal', formula=['"N"'], fill=red))

    if config.pad or config.keyboard:
        ws.conditional_formatting.add('G2:G9999', CellIsRule(
            operator='equal', formula=['"SSS"'], fill=grade_sss))
        ws.conditional_formatting.add('G2:G9999', CellIsRule(
            operator='equal', formula=['"SS"'], fill=grade_ss))
        ws.conditional_formatting.add('G2:G9999', CellIsRule(
            operator='equal', formula=['"S"'], fill=grade_s))
        ws.conditional_formatting.add('G2:G9999', CellIsRule(
            operator='equal', formula=['"A"'], fill=grade_a))
        ws.conditional_formatting.add('G2:G9999', CellIsRule(
            operator='equal', formula=['"B"'], fill=grade_b))
        ws.conditional_formatting.add('G2:G9999', CellIsRule(
            operator='equal', formula=['"C"'], fill=grade_c))
        ws.conditional_formatting.add('G2:G9999', CellIsRule(
            operator='equal', formula=['"D"'], fill=grade_d))
        ws.conditional_formatting.add('G2:G9999', CellIsRule(
            operator='equal', formula=['"F"'], fill=grade_f))

    if config.pad and config.keyboard:
        ws.conditional_formatting.add('K2:K9999', CellIsRule(
            operator='equal', formula=['"SSS"'], fill=grade_sss))
        ws.conditional_formatting.add('K2:K9999', CellIsRule(
            operator='equal', formula=['"SS"'], fill=grade_ss))
        ws.conditional_formatting.add('K2:K9999', CellIsRule(
    ...
    if scores:
        ...
        for s in scores_left:
            ...
            print("WARNING: New sheet does not contain a %s entry for CID=%d: %s %s" %
                  (etype, scores[s].cid, title, rstr))


def write_summary_sheet(ws, db, chart_set, config, mixId, pad):
    table_names = ["Single + Single Performance",
                   "Double + Double Performance", "Half-Double", "Routine", "Co-Op"]
    table_colors = ["ff2211", "11dd22", "cc0066", "23a98d", "f2c219"]
    table_headers = ["Passed", "Failed", "Unplayed", "SSS", "SS", "S",
                     "A", "B", "C", "D", "F", "Low Miss", "High Miss", "Avg Miss"]
    ...
    for r, (mode, _, diff, cid) in enumerate(table):
        ...
        if mode != None:
            ...
            ws.cell(row=r + 2, column=20, value='=IF(VLOOKUP(Q%d, Scores!$A$2:$M$9999, %d, FALSE)="", "", UPPER(VLOOKUP(Q%d, Scores!$A$2:$M$9999, %d, FALSE)))' %
                    (r + 2, score_col + 0, r + 2, score_col + 0))
            ws.cell(row=r + 2, column=21, value='=IF(VLOOKUP(Q%d, Scores!$A$2:$M$9999, %d, FALSE)="", "", UPPER(VLOOKUP(Q%d, Scores!$A$2:$M$9999, %d, FALSE)))' %
                    (r + 2, score_col + 1, r + 2, score_col + 1))
            ws.cell(row=r + 2, column=22, value='=IF(VLOOKUP(Q%d, Scores!$A$2:$M$9999, %d, FALSE)="", "", VLOOKUP(Q%d, Scores!$A$2:$M$9999, %d, FALSE))' %
                    (r + 2, score_col + 2, r + 2, score_col + 2))
        ...
        if last_mode != -1:
            ...
            if last_diff != diff or last_mode != mode:
                end_row = r + 1
                ws.cell(row=main_row - 1, column=2, value='=COUNTIF(T%d:T%d, "Y")' %
                        (start_row, end_row))
                ws.cell(row=main_row - 1, column=3, value='=COUNTIF(T%d:T%d, "N")' %
                        (start_row, end_row))
                ws.cell(row=main_row - 1, column=4, value='=(%d-%d+1) - B%d - C%d' %
                        (end_row, start_row, main_row - 1, main_row - 1))
                ws.cell(row=main_row - 1, column=5, value='=IF(COUNTIF(U%d:U%d, "SSS")<>0,COUNTIF(U%d:U%d, "SSS"),"")' %
                        (start_row, end_row, start_row, end_row))
                ws.cell(row=main_row - 1, column=6, value='=IF(COUNTIF(U%d:U%d, "SS")<>0,COUNTIF(U%d:U%d, "SS"),"")' %
                        (start_row, end_row, start_row, end_row))
                ws.cell(row=main_row - 1, column=7, value='=IF(COUNTIF(U%d:U%d, "S")<>0,COUNTIF(U%d:U%d, "S"),"")' %
                        (start_row, end_row, start_row, end_row))
                ws.cell(row=main_row - 1, column=8, value='=IF(COUNTIF(U%d:U%d, "A")<>0,COUNTIF(U%d:U%d, "A"),"")' %
                        (start_row, end_row, start_row, end_row))
                ws.cell(row=main_row - 1, column=9, value='=IF(COUNTIF(U%d:U%d, "B")<>0,COUNTIF(U%d:U%d, "B"),"")' %
                        (start_row, end_row, start_row, end_row))
                ws.cell(row=main_row - 1, column=10, value='=IF(COUNTIF(U%d:U%d, "C")<>0,COUNTIF(U%d:U%d, "C"),"")' %
                        (start_row, end_row, start_row, end_row))
                ws.cell(row=main_row - 1, column=11, value='=IF(COUNTIF(U%d:U%d, "D")<>0,COUNTIF(U%d:U%d, "D"),"")' %
                        (start_row, end_row, start_row, end_row))
                ws.cell(row=main_row - 1, column=12, value='=IF(COUNTIF(U%d:U%d, "F")<>0,COUNTIF(U%d:U%d, "F"),"")' %
                        (start_row, end_row, start_row, end_row))
                ws.cell(row=main_row - 1, column=13, value='=IF(SUMPRODUCT(--(V%d:V%d<>""))=0,"",MIN(V%d:V%d))' %
                        (start_row, end_row, start_row, end_row))
        ...
        if mode != None:
            ...
            if last_mode != mode:
                ...
                ws.cell(row=main_row, column=1).fill = PatternFill(
                    "solid", fgColor=table_colors[mode])
                for i, header in enumerate(table_headers):
                    ws.cell(row=main_row + 1, column=i + 2, value=header)
                    ws.cell(row=main_row + 1, column=i +
                ...
                for i in range(15):
                    ...
                    add_border(ws, main_row + 1, i + 1, top=Side(style="thin"),
                               bottom=Side(style="thin"))
                    add_border(ws, main_row, 1, left=Side(
                        style="medium"), right=Side(style="medium"))
                    add_border(ws, main_row + 1, 1, left=Side(style="medium"),
                               right=Side(style="thin"))
...
def read_scores(frompath):
    ...
    if not "Scores" in wb.sheetnames:
        raise Exception(
            "Old scores spreadsheet doesn't contain a sheet named 'Scores'. Did you rename it?")
    ws = wb["Scores"]
    ...
    if c == None:
        raise Exception(
            "Old 'Scores' sheet doesn't contain a column named 'CID'. Did you rename it?")
    ...
    if kbd_cols:
        for i in range(4):
            if not i in kbd_cols:
                print("WARNING: Keyboard scores available but column '%s' is missing" %
                      kbd_headers[i])
    ...
    for r in range(2, ws.max_row + 1):
        ...
        if pad_cols:
            ...
            if 0 in pad_cols:
                passed = ws.cell(row=r, column=pad_cols[0]).value
            if 1 in pad_cols:
                grade = ws.cell(row=r, column=pad_cols[1]).value
            if 2 in pad_cols:
                miss = ws.cell(row=r, column=pad_cols[2]).value
            if 3 in pad_cols:
                comment = ws.cell(row=r, column=pad_cols[3]).value
        ...
        if kbd_cols:
            passed = grade = miss = comment = None
            if 0 in kbd_cols:
                passed = ws.cell(row=r, column=kbd_cols[0]).value
            if 1 in kbd_cols:
                grade = ws.cell(row=r, column=kbd_cols[1]).value
            if 2 in kbd_cols:
                miss = ws.cell(row=r, column=kbd_cols[2]).value
            if 3 in kbd_cols:
                comment = ws.cell(row=r, column=kbd_cols[3]).value
...
def generate_xlsx(dbpath, outpath, configpath, frompath):
    ...
    for ispad in (True, False):
        if ispad == True and not config.pad:
            continue
        if ispad == False and not config.keyboard:
            continue
    ...
    ws_marker.cell(row=9, column=2, value="%d-%d%s" %
                   (config.diff_min, config.diff_max, ("", " (+Unrated)")[config.unrated]))
...
def pull_pump_out_db():
    sp.call(["git", "submodule", "init"])
    sp.call(["git", "submodule", "update"])
    db_file = sorted([f for f in os.listdir('pump-out-sqlite3-dump/dump/')
                      if f.endswith('db')])[-1]
    return os.path.join('pump-out-sqlite3-dump/dump/', db_file)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description="Create an XLSX spreadsheet for recording PIU scores.")
    parser.add_argument("out", type=str, help="The path of the spreadsheet to create")
    parser.add_argument(
        "--db", type=str, help="The path of the Pump Out database. If left unspecified, pull a new set of data")
    parser.add_argument("--from", type=str, dest="frompath",
                        help="The optional path of a previous spreadsheet from which to copy scores to the new one")
    parser.add_argument("--config", type=str, default="config.txt",
                        help="The path of the configuration file (default: config.txt)")
    parser.add_argument("--overwrite", action="store_true",
                        help="Overwrite the output path if it already exists (default: off)")
